...
- `out=` on `Engine.render` and `Voice.process`: a caller-provided float32 buffer (flat, `n * out_channels` samples for `render`) that the result is written into and returned, so a render loop can reuse one output buffer instead of allocating per call. It may be an ndarray. An undersized buffer is refused with the length required.

- `_core._pcm_decode` and `_core._pcm_encode_s16`: the sample-format half of WAV I/O in C — little-endian integer PCM at 1/2/3/4 bytes per sample in, 16-bit out. They back `softcut._wavio`, which pairs them with the stdlib `wave` module for the container.
...
- **numpy is no longer a dependency of any kind** — not required, not an extra. The extension allocates no results and imports nothing: `Engine.render` and `Voice.process` write into a buffer, and when the caller does not supply one the Python layer allocates an `array.array("f")`. numpy still works wherever a buffer is taken (a voice's buffer, render input, an `out` buffer) because an ndarray satisfies the buffer protocol, and `numpy.asarray` wraps what softcut returns without copying.

  Three visible consequences. `Engine.render` returns a **flat interleaved** `array.array("f")` of `n * out_channels` samples rather than an `(n, out_channels)` ndarray — `numpy.asarray(out).reshape(-1, eng.out_channels)` is the 2-D view, zero-copy — and `Voice.process` likewise returns a buffer. `Engine.allocate` and `NornsSoftcut.buffers` hand back `array.array("f")`. And a float64 input is now **refused with a message naming the problem** rather than silently converted, since the conversion was numpy's.

  `Engine.out_channels` is exposed on the facade to make the reshape expressible.
