...
Small, self-contained examples driving softcut with the audio files in `tests/data/`. Audio I/O uses only the standard library (`wave`) plus numpy, so no extra dependency is needed.

The offline demos render to WAV files under `build/out/` (out of source, cleaned by `make clean`) so you can listen to the results. Most also accept `--play` to play live to the speakers instead.
